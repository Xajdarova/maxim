...
public class StringProcessor
{
    public static async Task Main(string[] args)
    {
        ...
        else
        {
            ...
            int randomIndex = random.Next(processedString.Length);

            // Удаление символа в случайной позиции
            if (randomIndex >= 0 && randomIndex < processedString.Length)
            {
                string trimmedString = RemoveCharacterAt(processedString, randomIndex);
                Console.WriteLine("«Урезанная» обработанная строка:");
                Console.WriteLine(trimmedString);
            }
            else
            {
                Console.WriteLine("Ошибка: Не удалось получить корректный индекс для удаления символа.");
            }
        }
    }
